...
\todo{Describe the classical block, evaluated on exactly the same items as
...
\todo{Phase P10. This is the principal focus of the work. Describe the
...
\todo{Phase P10. The convolutional backbone shared with \texttt{siamese\_mel},
...
\todo{Describe the measure registry: cosine as the canonical measure,
...
\todo{Phase P7, applied on top of any representation, so that the gain
...
F1--F4 are in scope: F1, F3 and F4 are core, while the rule-based route F2 is an
optional extension the supervisor did not endorse.}
